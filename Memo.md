...
## 인터페이스
인터페이스는 클래스 혹은 프로그램이 제공하는 기능을 명시적으로 선언하는 역할을 함
추상 메서드와 상수로만 이루어져 있으며, 구현된 코드가 없기 때문에 인스턴스를 생성할 수도 없음
패키지에서 New->interface로 만든 파일: 인터페이스에서 선언한 변수는 컴파일 과정에서 `public static final` 상수로 변환되고, 메서드는 `public astract` 추상 메서드로 변환됨!
선언된 인터페이스를 클래스가 사용하는 것을 '클래스에서 인터페이스를 구현한다'라고 표현함 -> `implements` 예약어 사용
인터페이스의 역할은 인터페이스를 구현한 클래스가 어떤 기능의 메서드를 제공하는지 명시하는 것임, 그에 따라서 클라이언트는 인터페이스에서 약속한 명세만 보고도 구현된 클래스를 바로 사용할 수 있음
인터페이스는 한 클래스가 여러 인터페이스를 구현할 수 있음
### 디폴트 메서드
자바 8부터 가능
디폴트 메서드는 인터페이스에서 구현 코드까지 작성한 메서드임, 인터페이스를 구현한 클래스에 기본적으로 제공할 메서드임!
```
default void 함수명() { }
```
하위 클래스에서 재정의 가능함
### 정적 메서드
자바 8부터 가능
정적 메서드는 인스턴스 생성과 상관없이 사용할 수 있는 메서드임
```
static void 함수명() { } // 구현 시
인터페이스 이름.함수이름(); // 호출 시
```
### private 메서드
자바 9부터 가능
private 메서드는 인터페이스를 구현한 클래스에서 사용하거나 재정의할 수 없음
따라서 기존에 구현된 코드를 변경하지 않고 구현한 클래스에서 공통으로 사용하는 경우 private 메서드로 구현함
또는 클라이언트 프로그램에 제공할 기본 기능으로 구현하기도 함